...
- 每篇 `docs/*.md` 顶部需维护“摘要”块，统一样式为二级标题：`## 摘要：`（注意两个 #）。亦可使用 `<!-- SUMMARY-START --> ... <!-- SUMMARY-END -->` 注释包裹作为权威来源。
...
- 对齐指令入口：`python scripts/align_docs.py`
  - 依次执行：
  1) `scripts/ensure_docs_style_from_date.py`（根据文内“日期”与标题规范文首样式，并将文件名重命名为“时间戳_标题.md”，其中时间戳取文内日期的本地 00:00:00 秒；同日冲突按标题名降序依次向前一秒分配；同时将任何 `### 摘要：`/更深层级标题统一规范为 `## 摘要：`，并在缺失中文冒号时自动补全）；
...
- 文首结构：
  ...
  - （可选）O3 注释块
  - 空一行
  - `## 摘要：` 与摘要正文（注意是两个 `#` 的二级标题；若缺失 `：` 将自动补全）
  - 后续正文与分隔线 `---` 等不受此条影响
