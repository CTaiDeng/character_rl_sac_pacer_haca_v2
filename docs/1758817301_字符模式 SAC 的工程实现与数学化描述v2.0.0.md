...
## 2 形式化定义
- 词表并集（只读）：
  $$\mathcal{C}=\text{Catalog}=\texttt{chinese\_name\_frequency\_word.json}\ \cup\ \texttt{chinese\_frequency\_word.json}.$$
- 允许长度集合：
  $$U=\text{union.lengths}\subset\mathbb{N},\quad \text{来自}\ \texttt{data/word\_length\_sets.json}.$$
...
## 6 复杂度与实现考量
- 命中判定：每次 $O(|U|)$，实际 $|U|\approx 10\text{–}12$，结合缓存可控。
...
## 9 评测建议
- 消融：固定 $U=\{2\}$ vs. $U=\text{union.lengths}$；比较收敛速度、最终 reward、词法命中率。
- 命中统计：平均命中长度、命中率（前缀/后缀分别统计）。
- 泛化：跨域/OOV 文本的表现差异，长词边界识别能力。

附录：记号
- $\mathcal{C}$：词表并集；$U$：长度集合；$\chi_t$：目标字符；$\mathrm{prev}_t$：摘要预览；$\mathrm{source}_t$：源串；$q$：raw\_action 串；$s$：bigram 串；
