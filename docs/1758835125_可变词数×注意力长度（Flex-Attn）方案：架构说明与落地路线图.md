...
* 词法：`word_noncompliance` ↓≥30%；合法词覆盖 ↑。
...
| 风险          | 触发机制                      | 对策                              |           |                           |
| ----------- | ------------------------- | ------------------------------- | --------- | ------------------------- |
| 长度偏置（越长越有利） | 长词命中奖励放大                  | $L_p$ 上限 + 长度成本 + 语义门控 + IDF 降权 |           |                           |
| 词典投机        | 高频词堆砌                     | 禁单字奖励、二字降权、门控阈值 $\tau$ 动态调优     |           |                           |
| 训练/推理分布偏移   | 训练期 Top-p                 | 训练禁 Top-p；上线前 Eval-w/o-Top-p 校准 |           |                           |
| 信息泄露        | $\chi_t$ 进演员              | 仅评论家/奖励可见；演员侧剔除                 |           |                           |
| 吞吐下降        |                            U                               | 扫描、日志 I/O | 反向 Trie/AC + 缓存；日志批量/采样写入 |
| 长度崩塌        | $\pi_{L_h},\pi_{L_p}$ 单点化 | 熵正则 + 混采 + 一致性正则                |           |                           |
