...
- 配置化落地：新增 $\texttt{topology\_word\_packs}$、$\texttt{forward\_pack\_match.*}$、$\texttt{backward\_iter.*}$ 配置段，默认兼容 v2 行为。
...
匹配语义
- $\texttt{match\_phrase}$ 默认精确匹配；可在配置中切换到“最长可用/别名归一/大小写/全半角”规则。
- $\texttt{scope}$ 支持 $\texttt{tail}$ 与 $\texttt{substring}$；生产默认 $\texttt{tail}$ 以对齐 v2 习惯与复杂度上界。

复杂度（单步）
$$\mathcal{O}(|U|\cdot \bar{k})\quad \text{或 }\ \mathcal{O}(|\mathrm{sub}(s,U)|\cdot \bar{k})\ (\text{子串作用域}),$$
其中 $\bar{k}$ 为词包内平均候选数，$\texttt{sub(s,U)}$ 为受 $U$ 约束的候选子串集。
...
注：$\texttt{sample\_next}$ 可为贪心、温度采样或 Top‑p，仍服从合规模块的掩码约束。

---

## 4. 配置项（新增）

建议在 $\texttt{res/config.json}$/$\texttt{config\_template.json}$ 中新增：
...
并新增数据文件示例 $\texttt{data/topology\_word\_packs.json}$：
...
兼容性：当 $\texttt{packs\_path}$ 为空或解析失败时，前向命中退化为 v2 的 Catalog 单词命中；$\texttt{k\_max=2, stop\_on\_hit=true}$ 时，后向扩展近似 v2 bigram。

---

## 5. 与合规模块/奖励的接口

- 合规模块 $\texttt{\_mask\_logits}$ 仅需将“基于 H 的命中检查”作为可选诊断信号，不改变禁用/允许集的语义。
- 奖励日志：保留 $\texttt{lexical\_bigram\_bonus}$ 字段名，但记录更一般的：$\texttt{lexical\_topo\_bonus}$、$\texttt{topo\_hit\_phrase}$、$\texttt{topo\_pack\_id}$、$\texttt{iter\_k}$。
- 评分延续 v2：
  $$
  \delta_t=\begin{cases}
  1.0,&\text{命中拓扑词（包/单词）},\\
...
- 复杂度：词包匹配令 $\bar{k}$ 增大，建议为 $\texttt{packs}$ 建立 Trie/AC 自动机或散列桶以维持均摊 $\mathcal{O}(1)$ 命中；子串作用域慎用。
- 数据预处理：将常用短语与别名统一到 $\texttt{packs}$，并在规范化流程中做别名归一（与 $\texttt{normalize}$ 同步）。
- 可观测性：在 HTML/CSV 可视化中新增列：$\texttt{pack\_hit}$、$\texttt{pack\_id}$、$\texttt{hit\_len}$、$\texttt{iter\_len}$，便于离线分析。

---

## 7. 实现映射（代码位置建议）

- $\texttt{src/character\_sac\_trainer.py}$
  - 新增/改造：
    - $\texttt{TopologyWordPacks}$ 装载与规范化（读取 $\texttt{packs\_path}$）。
    - $\texttt{PACK\_HIT}$ 与 $\texttt{FORWARD\_PACK\_HIT}$；$\texttt{ITER\_BACKWARD\_EXTEND}$；
    - 日志与奖励记录字段扩展。
  - 兼容：当未启用新特性时，路径回落到 v2 的 $\texttt{FORWARD\_EXTEND\_BIGRAM}$/后缀命中逻辑。

---

## 8. 回滚与 A/B

- 参数回滚：$\texttt{enabled=false}$ 即回到 v2 行为；$\texttt{k\_max=2}$ 近似 bigram；$\texttt{packs\_path}$ 置空即退化为单词。
- A/B 方案：对 $\texttt{packs}$ 的不同定义做流量切分，观察 $\texttt{lexical\_topo\_bonus}$ 分布与收敛稳定性差异。
