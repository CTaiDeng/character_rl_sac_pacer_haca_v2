### 字符粒度策略环境：形式化建模与离散 SAC

> 定位：去泄漏、强约束、可运营。面向生产的**字符级 POMDP**与**离散最大熵 SAC**训练方案，内置**双缓冲 + BC**并轨、**Top-K 期望近似**、**硬掩码合规**与**在线可观测 KPI**。本稿为独立完备版。
...
本方案已将恒零项剔除，保留该定理以兼容后续跨粒度共享回报的需要。
...
* 移除 $o_t$ 中的 $\tau_t$ 暴露；
* 将温度惩罚/偏置合规模块**改为硬掩码**；
* 连续版目标式样改为**离散期望**（可 Top-K）；
* 引入 $\log\alpha$ 自适应，目标熵随 $|\mathcal{A}(s)|$；
* 新增双缓冲、BC、DAgger 调度；
* 奖励体系换为 n-gram 覆盖 + InfoNCE，对齐段落目标。
