...
- 在 `README.md` 文末追加 `docs` 目录下每篇文章的“摘要”索引（只读复制，不修改原文档）。索引明确排除 `docs/LICENSE.md`。
- 每篇 `docs/*.md` 顶部需维护“摘要”块，统一样式为二级标题：`## 摘要`（注意两个 #，且不带冒号）。亦可使用 `<!-- SUMMARY-START --> ... <!-- SUMMARY-END -->` 注释包裹作为权威来源。
- 摘要长度要求：尽量完整，建议 120–300 字（约 3–6 句）；可包含 3–5 条要点列表补充关键信息。
- 摘要来源优先级：1）`<!-- SUMMARY-START -->...<!-- SUMMARY-END -->` 注释块；2）文中 `## 摘要` 标题后的首段正文；3）以 `> 摘要：` 开头的连续引文行（兼容旧文档）。
...
- 原则：未显式给出“项目相对路径”的情况下，任何指令不得修改 `docs/` 下文章内容。
  - 项目相对路径示例：`docs/1758365560_将阅读理解形式化为“认知资本”的交易与增值过程：基于传统数学的严格论证.md`
  - 仅列出目录（如 `docs/`）、通配符或空参数均视为“未显式给出路径”。
- 脚本执行约束：所有可能改动 `docs/` 的脚本均启用“路径显式校验”，必须传入待处理文件的项目相对路径列表；否则脚本将拒绝执行并退出。
  - 受约束脚本（示例，不限于）：
    - `scripts/ensure_docs_style_from_date.py <files...>`
    - `scripts/insert_docs_license_footer.py <files...>`
    - `scripts/force_docs_utf8_bom.py <files...>`
    - `scripts/insert_doc_date_from_prefix.py <files...>`
    - `scripts/ensure_title_equals_filename.py <files...>`
  - 只读脚本（不会改动文章，如 `scripts/update_readme_index.py`）不受此限制。
- 对齐入口：`python scripts/align_docs.py [<files...>]`
...
- 文件命名：`<秒时间戳>_<标题>.md`。其中“秒时间戳”由文档内“日期：YYYY-MM-DD”换算为本地 00:00:00 的 Unix 秒数；同一日期内按标题名（去前缀与扩展名）降序排序，从当日的第一秒起逐秒递增分配：`ts+1, ts+2, ts+3, ...`（确保同目录内唯一且稳定）。
- 文首结构：
  - 第一行：`# <标题>`
  - 空一行
  - 元信息三行（相邻无空行）：
    - `- 作者：GaoZheng`
    - `- 日期：YYYY-MM-DD`
    - `- 版本：vX.Y.Z`（若缺失则初始化为 `v1.0.0`，若已存在则保留）
  - 元信息后空一行
  - （可选）O3 注释块
  - 空一行
  - `## 摘要` 与摘要正文（注意是两个 `#` 的二级标题；不带冒号）
  - 后续正文与分隔线 `---` 等不受此条影响
...
- 行尾与编码：全仓统一使用 UTF-8（无 BOM）+ LF；任何导出/生成的文本文件不得使用 CRLF 或 UTF-8 BOM。既有“UTF8+CRLF”策略一律改为“UTF-8+LF”。
- .gitattributes：根目录采用统一规范 `* text=auto eol=lf`，为脚本/源码显式指定 LF；对二进制类型设为 `binary`，对只读目录 `docs/kernel_reference/` 设为 `-text`（不作归一化）。
